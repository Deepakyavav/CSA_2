...
token_patterns = {
    'BRANCH_CONDITION': r'Branch\s+Condition:\s+[a-zA-Z]+,\s+Target:',  # Matches branch condition
    'TARGET': r'Target:\s+[a-zA-Z0-9_]+(\s*[,a-zA-Z0-9_()%$@.-]*)*',  # Matches target instruction
        ...
        for token_type, pattern in token_patterns.items():
            ...
            if match:
                ...
                if token_type != 'WHITESPACE' and token_type != 'COMMENT':
                    tokens.append(Token(token_type, value.strip()))
                code = code[match.end():]
                break
        if not match:
...
def parser(tokens):
    ...
    while tokens:
        token = tokens.pop(0)
        if token.type == 'BRANCH_CONDITION':
            branch_condition = token.value.split(',')[0].strip()
            parse_tree.append((branch_condition, []))
        elif token.type == 'TARGET':
            if parse_tree:  # Check if parse tree is not empty
                parse_tree[-1][1].append(token.value.split(':')[1].strip())
            else:
...
def output_parse_tree(parse_tree, filename):
    with open(filename, 'w') as file:
        for branch_condition, targets in parse_tree:
            file.write('Branch Condition: {}\n'.format(branch_condition))
...
assembly_file_path = 'branch_conditions.txt'
